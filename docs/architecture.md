...
Kubernetes dependencies are pinned to `1.37.0-rc.1`. That source dependency
choice is separate from the stable `1.37.0` server/kubelet profile used in
historical live validation. Nested modules retain their own dependency pins.
...
and historical evidence boundaries, see [provenance](provenance.md).
