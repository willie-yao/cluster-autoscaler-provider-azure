...
The inherited chart version `9.59.0` and `appVersion: 1.35.0` are unchanged.
A chart-changing PR without a version increment remains subject to that
existing gate. Repository owners decide versioning and publication; a local
compatibility pass does not waive the gate or approve a release.
...
execution and operator-owned infrastructure cleanup. Historical results apply
only to their recorded test/runtime checkpoints, as described in
